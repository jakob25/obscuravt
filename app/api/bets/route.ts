...
export async function POST(req: NextRequest) {
  ...
  const titleWords = new Set<string>(title.toLowerCase().split(/\s+/).filter((w: string) => !stopWords.has(w)))

  const dupes = (existing ?? []).filter((b: { title: string }) => {
    const bWords = new Set<string>(b.title.toLowerCase().split(/\s+/).filter((w: string) => !stopWords.has(w)))
