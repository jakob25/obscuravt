...
export default function LoginPage() {
  ...
  const boltsIn = () => {
    ['bT','bB','bL','bR','bTR','bTL','bBR','bBL'].forEach(id => {
      const el = document.getElementById(id)
      if (el) el.style.transform = ''
    })
    const ring = document.getElementById('dialRing')
    if (ring) ring.style.animationDuration = '10s'
  }
      ...
      <div className="vl-page">
        <div className="vl-card">
          ...
          <div className="vl-lock">
            ...
            {/* Login fields inside center circle */}
            <div className="fo-wrap">
              <div className="fo-tabs">
                <button className={`fo-tab ${mode === 'login' ? 'on' : ''}`} onClick={() => { setMode('login'); setError('') }}>Sign In</button>
                <button className={`fo-tab ${mode === 'register' ? 'on' : ''}`} onClick={() => { setMode('register'); setError('') }}>Register</button>
              </div>
