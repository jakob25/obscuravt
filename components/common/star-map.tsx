...
const SCATTER: Record<string, { x: number; y: number }> = {
  clust_parasocial:   { x: 500, y: 390 }, // fixed center per design mandate
  clust_idol:         { x: 480, y: 95  },
  clust_variety:      { x: 810, y: 160 },
  clust_gaming:       { x: 900, y: 370 },
  clust_chaos:        { x: 780, y: 620 },
  clust_horror:       { x: 430, y: 700 },
  clust_menhara:      { x: 130, y: 600 },
  clust_denpa:        { x: 80,  y: 370 },
  clust_stem:         { x: 100, y: 175 },
  clust_comfy:        { x: 270, y: 80  },
  clust_worker:       { x: 650, y: 520 },
  clust_vsinger:      { x: 950, y: 510 },
  clust_experimental: { x: 340, y: 290 },
}
...
export function StarMap({ onVTuberSelect, onConstellationSelect, initialConstellation }: StarMapProps) {
  ...
  useEffect(() => {
    ...
    constellations.forEach((constellation) => {
      ...
      const pos = SCATTER[constellation.id] ?? { x: constellation.position.x, y: constellation.position.y }
      const centerX = pos.x
      const centerY = pos.y
  ...
  useEffect(() => {
    ...
    if (initialConstellation) {
      ...
      if (constellation) {
        const tpos = SCATTER[constellation.id] ?? { x: constellation.position.x, y: constellation.position.y }
        const targetX = tpos.x
  ...
  const handleMouseMove = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (transform.k >= ZOOM_THRESHOLD) {
      ...
      const hoveredConst = constellations.find(constellation => {
        const hpos = SCATTER[constellation.id] ?? { x: constellation.position.x, y: constellation.position.y }
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (showConstellationLabels) {
        constellations.forEach(constellation => {
          const spos = SCATTER[constellation.id] ?? { x: constellation.position.x, y: constellation.position.y }
          const centerX = spos.x
          const centerY = spos.y
